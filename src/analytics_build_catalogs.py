...
def addCompanies(jobid, fromcompany, tocompany):
    ...
    q2 = cndb.query(LIExperience.nrmCompany.label('nrm'),
    ...
    if tocompany is not None:
        ...
        q2 = q2.filter(LIExperience.nrmCompany < tocompany)
...
def addInstitutes(jobid, frominstitute, toinstitute):
    ...
    q = cndb.query(LIEducation.nrmInstitute, LIEducation.institute,
                   func.count(LIEducation.id)) \
            .filter(LIEducation.nrmInstitute >= frominstitute)
    if toinstitute is not None:
        q = q.filter(LIEducation.nrmInstitute < toinstitute)
    q = q.group_by(LIEducation.nrmInstitute, LIEducation.institute) \
         .order_by(LIEducation.nrmInstitute)
...
def addDegrees(jobid, fromdegree, todegree):
    ...
    q = cndb.query(LIEducation.nrmDegree, LIEducation.degree,
                   func.count(LIEducation.id)) \
            .filter(LIEducation.nrmDegree >= fromdegree)
    if todegree is not None:
        q = q.filter(LIEducation.nrmDegree < todegree)
    q = q.group_by(LIEducation.nrmDegree, LIEducation.degree) \
         .order_by(LIEducation.nrmDegree)
...
def addSubjects(jobid, fromsubject, tosubject):
    ...
    q = cndb.query(LIEducation.nrmSubject, LIEducation.subject,
                   func.count(LIEducation.id)) \
            .filter(LIEducation.nrmSubject >= fromsubject)
    if tosubject is not None:
        q = q.filter(LIEducation.nrmSubject < tosubject)
    q = q.group_by(LIEducation.nrmSubject, LIEducation.subject) \
         .order_by(LIEducation.nrmSubject)
...
if catalog is None or catalog == 'titles':
    ...
    q2 = cndb.query(LIExperience.nrmTitle).filter(LIExperience.nrmTitle != None)
    if startval:
        q1 = q1.filter(LIProfile.nrmTitle >= startval)
        q2 = q2.filter(LIExperience.nrmTitle >= startval)
    q = q1.union(q2)
...
if catalog is None or catalog == 'companies':
    ...
    q2 = cndb.query(LIExperience.nrmCompany) \
             .filter(LIExperience.nrmCompany != None)
    if startval:
        q1 = q1.filter(LIProfile.nrmCompany >= startval)
        q2 = q2.filter(LIExperience.nrmCompany >= startval)
    q = q1.union(q2)
...
if catalog is None or catalog == 'institutes':
    ...
    q = cndb.query(LIEducation.nrmInstitute) \
            .filter(LIEducation.nrmInstitute != None)
    if startval:
        q = q.filter(LIEducation.nrmInstitute >= startval)
...
if catalog is None or catalog == 'degrees':
    ...
    q = cndb.query(LIEducation.nrmDegree).filter(LIEducation.nrmDegree != None)
    if startval:
        q = q.filter(LIEducation.nrmDegree >= startval)
...
if catalog is None or catalog == 'subjects':
    ...
    q = cndb.query(LIEducation.nrmSubject) \
            .filter(LIEducation.nrmSubject != None)
    if startval:
        q = q.filter(LIEducation.nrmSubject >= startval)
