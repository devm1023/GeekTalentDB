...
def addUWProfile(dtdb, uwprofiledoc, dtsession, logger):
    ...
    for experience in dtsession.query(
            url=conf.DATOIN_PROFILES+'/'+uwprofile_id+'/experiences',
            params={}, batchsize=20):
    ...
    for education in dtsession.query(
            url=conf.DATOIN_PROFILES+'/'+uwprofile_id+'/educations',
            params={}, batchsize=20):
    ...
    for test in dtsession.query(
            url=conf.DATOIN_PROFILES+'/'+uwprofile_id+'/tests',
            params={}, batchsize=20):
...
def downloadProfiles(fromTs, toTs, offset, rows, byIndexedOn, sourceId):
    ...
    for attempt in range(conf.MAX_ATTEMPTS):
        ...
        for offset in failed_offsets:
            ...
            try:
                liprofiledoc \
                    = next(dtsession.query(url=conf.DATOIN_SEARCH,
                                           params=params,
                                           rows=1,
                                           offset=offset))
    ...
    try:
        ...
        while sys.argv:
            ...
            if len(option) == 1:
                ...
                option=option[0]
                if option == '--source':
                    if value in ['linkedin', 'indeed', 'upwork']:
                        sourceId = value
                    else:
                        raise ValueError('Invalid command line argument.')
                elif option == '--offset':
