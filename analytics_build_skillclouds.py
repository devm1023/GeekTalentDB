...
def addCompanySkills(fromcompany, tocompany,
                     companycountthreshold, skillcountthreshold, countthreshold):
    andb = AnalyticsDB(conf.ANALYTICS_DB)
    logger = Logger(sys.stdout)
    count = func.count(ExperienceSkill.experienceId).label('counts')
    q = andb.query(Experience.nrmCompany,
            ...
            .join(Skill) \
            .join(Company) \
            .filter(Company.experienceCount >= companycountthreshold) \
            .filter(Skill.experienceCount >= skillcountthreshold) \
...
def addSkillSkills(fromskill, toskill,
                   skill1countthreshold, skill2countthreshold, countthreshold):
    andb = AnalyticsDB(conf.ANALYTICS_DB)
    logger = Logger(sys.stdout)
    ...
    q = andb.query(ExperienceSkill1.nrmSkill,
            ...
            .filter(Skill1.experienceCount >= skill1countthreshold) \
            .filter(Skill2.experienceCount >= skill2countthreshold) \
...
if category is None or category == 'titles':
...
elif category is None or category == 'companies':
    companythreshold = 10
    skillthreshold = 10
    countthreshold = 3
    ...
    splitProcess(q, addCompanySkills, batchsize,
                 njobs=njobs, logger=logger,
                 args=[companythreshold, skillthreshold, countthreshold],
                 workdir='jobs', prefix='build_company_skillclouds')
elif category is None or category == 'skills':
    skill1threshold = 10
    skill2threshold = 10
    countthreshold = 3
    ...
    splitProcess(q, addSkillSkills, batchsize,
                 njobs=njobs, logger=logger,
                 args=[skill1threshold, skill2threshold, countthreshold],
